...
| user_id | UUID | Relation â†’ Users | Foreign key linking to the user who recorded the memo. |
...
| narrative_function | TEXT/ENUM | Select | Label describing the memoâ€™s narrative function (scene, emotion_dump, meaning_making, decision_point, integration_reflection, planning_future). |
| cognitive_state | TEXT/ENUM | Select | Cognitive state such as overloaded, scattered, ruminating, calm, focused, numb or shutdown. |
| embodiment_state | TEXT/ENUM | Select | Degree of somatic engagement: hypercognitive_disembodied, head_heavy_body_ignored, partially_embodied, grounded, somatically_overwhelmedã€450110501613372â€ L0-L152ã€‘. |
| time_reference | TEXT/ENUM | Select | Past, present, future or mixed, reflecting the orientation of the narrative . |
| flavor_tags | TEXT[]/JSONB | Multiâ€‘select | Tags from the Texture &â€¯Flavor framework (SEE/FEEL/HEAR categories). |
| meta_themes | TEXT[]/JSONB | Multiâ€‘select | Higherâ€‘level thematic labels (trauma, conflict, hope, shame, absurdity, etc.). |
| reframe_opportunity_score | NUMERIC(3,2) | Number (0â€“1) | Score indicating how ripe the memo is for reframing; higher means greater potential. |

# **Taxonomies and Classification Logic**

**Emotional Taxonomy**

The classifier uses a variant of Plutchik’s wheel, which posits eight primary emotions arranged in opposing pairs.  Emotions can vary in intensity (e.g., annoyance → anger → rage) and blend with adjacent emotions .  For the MVP, the primary set includes joy, sadness, anger, fear, trust, disgust, anticipation, surprise, anxiety and frustration.

**Narrative Function Taxonomy**

Drawing on narrative theory, memos are tagged by their function: scene (describes events), emotion_dump (expresses feelings without a clear plot), meaning_making (reflects on meaning), decision_point (where a choice is made), integration_reflection (connecting past insights), and planning_future (anticipating or setting goals).  These categories mirror Labov’s narrative structure of orientation, complication and evaluation .
...
The system infers whether the narrative centres on the past, present, future or a mix.  Research suggests that present orientation fosters a sense of control while past or future orientation can be associated with rumination or anxiety .  The classifier detects verb tenses and explicit temporal markers to assign a time reference.
...
| **Method** | **Endpoint** | **Description** | **Request Body/Parameters** | **Response** |
| --- | --- | --- | --- | --- |
| POST | /voice-memos | Upload a voice memo (audio and metadata). | Multipart form: audio file or audio_url, user_id, optional timestamp.Â  Server will transcribe and classify automatically. | 201 Created with memo object. |
...
| **Method** | **Endpoint** | **Description** | **Request Body/Parameters** | **Response** |
| --- | --- | --- | --- | --- |
| GET | /clusters | Return mapping of metaâ€‘themes to memo IDs. | Optional query: meta_theme=conflict. | 200 OK with cluster object. |
...
5. Prioritise Present Orientation – Identify whether the user is focused on the present (in control), the past (rumination) or the future (anticipation) , and encourage shifts toward present‑focused perspectives where appropriate.
...
├── backend/
│   ├── [app.py](http://app.py/)                 # FastAPI or Flask application with API endpoints
│   ├── models/                # SQLAlchemy models for Users and VoiceMemos
│   ├── schemas/               # Pydantic schemas for request/response bodies
│   ├── services/
│   │   ├── [ingestion.py](http://ingestion.py/)       # Handles audio upload and transcription
│   │   ├── [classifier.py](http://classifier.py/)      # Contains classification logic and taxonomies
│   │   └── [reports.py](http://reports.py/)         # Generates cluster maps and monthly reports
...
- Narrative structure components (orientation, complication, evaluation) that inform narrative function tagging .
...
- Research on temporal orientation, noting how focus on past, present or future influences mood and control .
- The eight primary emotions and their intensity scales used to construct the emotional taxonomy .
